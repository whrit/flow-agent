...
async function runInteractiveWizard() {
    ...
    try {
        ...
        const objective = 'Build a modern web application';
        ...
        await new Promise((resolve, reject)=>{
            db.serialize(()=>{
                const insertSwarm = db.prepare(`
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                `);
                insertSwarm.run(swarmId, `hive-${Date.now()}`, objective, 'active', config.coordination, new Date().toISOString(), new Date().toISOString());
                const insertAgent = db.prepare(`
                    INSERT INTO agents (id, swarm_id, name, type, role, status, capabilities, created_at)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                `);
                insertAgent.run(queenId, swarmId, 'Queen Coordinator', 'coordinator', 'queen', 'active', JSON.stringify([
                    'orchestration',
                    'strategy',
                    'coordination'
                ...
                for(let i = 0; i < config.agents - 1; i++){
                    ...
                    insertAgent.run(`agent-${Date.now()}-${i}`, swarmId, `${agentType.charAt(0).toUpperCase() + agentType.slice(1)} Worker ${i + 1}`, agentType, 'worker', 'idle', JSON.stringify([
                        agentType,
                        'collaboration'
                    ]), new Date().toISOString());
                }
...
//# sourceMappingURL=hive-mind-wizard.js.map