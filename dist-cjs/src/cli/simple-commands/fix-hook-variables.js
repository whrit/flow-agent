...
function transformHookCommand(command, fromSyntax, toSyntax) {
    ...
    if (toSyntax === 'wrapper') {
        const scriptName = command.includes('post-edit') ? 'post-edit-hook.sh' : command.includes('pre-edit') ? 'pre-edit-hook.sh' : 'generic-hook.sh';
...
`;
            await fs.writeFile(path.join(hooksDir, 'post-edit-hook.sh'), script, {
                mode: 0o755
            });
...
export const fixHookVariablesCommandConfig = {
    ...
    options: [
        ...
        {
            flag: '--syntax <type>',
            description: 'Force specific syntax: environment, jq, wrapper'
        },
        {
            flag: '--test',
            description: 'Create test hook configuration'
        }
    ],
    examples: [
