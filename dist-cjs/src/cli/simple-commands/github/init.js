...
`;
    await writeFile(checkpointHooksPath, content, {
        mode: 0o755
    });
...
export async function githubInitCommand(flags = {}) {
    try {
        ...
        const checkpointManagerContent = await readFile(join(dirname(import.meta.url).replace('file://', ''), '..', 'init', 'templates', 'commands', 'helpers', 'checkpoint-manager.sh'), 'utf8').catch(()=>{
            return `#!/bin/bash
# Checkpoint manager for Claude Flow
# Use github-checkpoint-hooks.sh for checkpoint operations
`;
        });
        await writeFile(checkpointManagerPath, checkpointManagerContent, {
            mode: 0o755
        });
