...
export class BatchOptimizer {
    static calculateOptimalConcurrency(projectCount, systemSpecs = {}) {
        const { cpuCores = 4, memoryGB = 8, diskSpeed = 'ssd' } = systemSpecs;
        let optimal = Math.min(cpuCores * 2, Math.floor(memoryGB / 0.5), projectCount, 20);
        if (diskSpeed === 'hdd') {
            optimal = Math.ceil(optimal * 0.7);
        }
        return Math.max(1, optimal);
    }
    static estimateCompletionTime(projectCount, options = {}) {
        const { concurrency = 5, template = 'basic', sparc = false, averageTimePerProject = 15 } = options;
