...
export class ToolExecutionFramework {
    ...
    initializeFormatters() {
        ...
        this.resultFormatters.set('default', (result)=>({
                ...
                details: Object.entries(result).map(([key, value])=>`${key}: ${typeof value === 'object' ? JSON.stringify(value) : value}`),
                status: result.success ? 'success' : 'error'
            }));
    }
    ...
    async executeToolDirect(execution) {
        ...
        try {
            ...
            const result = await this.mcpLayer.executeTool(execution.toolName, execution.parameters, execution.options);
    ...
    async executeToolsBatch(toolExecutions, options = {}) {
        ...
        try {
            if (options.parallel) {
                const promises = toolExecutions.map(({ toolName, parameters, toolOptions })=>this.executeTool(toolName, parameters, toolOptions));
...
//# sourceMappingURL=tool-execution-framework.js.map