...
async function getHiveMindMetrics() {
    ...
    if (!existsSync(dbPath)) {
        return {
            available: false,
            reason: 'Hive-mind database not found'
        };
    }
    try {
...
async function getRuvSwarmMetrics() {
    ...
    if (!existsSync(dbPath)) {
        return {
            available: false,
            reason: 'ruv-swarm database not found'
        };
    }
    try {
        const db = new Database(dbPath);
...
async function integrateMetrics(hiveMind, ruvSwarm) {
    ...
    if (integration.systems.length > 0) {
        ...
        integration.combined.success_rate = integration.combined.total_tasks > 0 ? (integration.combined.completed_tasks / integration.combined.total_tasks * 100).toFixed(1) : '0';
    }
    return integration;
}
