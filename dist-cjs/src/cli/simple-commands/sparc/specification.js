...
export class SparcSpecification extends SparcPhase {
    ...
    extractKeywords() {
        ...
        return words.filter((word)=>word.length > 2 && !stopWords.includes(word)).filter((word, index, arr)=>arr.indexOf(word) === index).slice(0, 10);
    }
    extractActionVerbs() {
        const verbs = [
            'create',
    ...
    async estimateTimeline(requirements) {
        const totalStoryPoints = requirements.length * 3;
        const developmentVelocity = 10;
