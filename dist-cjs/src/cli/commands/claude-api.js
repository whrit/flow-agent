...
export const claudeApiCommand = new Command().name('claude-api').description('Manage Claude API configuration and test connectivity').action(()=>{
...
claudeApiCommand.command('test').description('Test Claude API connectivity').option('--model <model>', 'Model to test').option('--temperature <temp>', 'Temperature for test', parseFloat).option('--prompt <prompt>', 'Test prompt', 'Hello, Claude! Please respond with a brief greeting.').action(async (options)=>{
    try {
        const configManager = ConfigManager.getInstance();
