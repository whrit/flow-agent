    ...
    async list (args, options = {}) {
        try {
            ...
            for (const agent of agents){
                ...
                const statusEmoji = agent.status === 'idle' ? '⭕' : agent.status === 'busy' ? '🔵' : agent.status === 'error' ? '🔴' : '⚪';
...
export const { spawn: spawnAgent, list: listAgents, info: agentInfo, terminate: terminateAgent, start: startAgent, restart: restartAgent, health: agentHealth } = agentCommands;
