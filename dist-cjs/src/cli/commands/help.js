...
export const helpCommand = new Command().name('help').description('Show help information').argument('[command]', 'Command to show help for').option('--all', 'Show all available commands').action(async (command, options)=>{
    if (command) {
...
function showCommandHelp(command) {
    ...
    if (commandHelp) {
        ...
        console.error(HelpFormatter.formatError(`Unknown command: ${command}`, 'claude-flow help', 'claude-flow help [command]'));
    }
}
    ...
    const commandHelpMap = {
        'hive-mind': {
            ...
            commands: [
                ...
                {
                    name: 'status',
                    description: 'View active swarms and metrics'
                },
                {
                    name: 'stop',
