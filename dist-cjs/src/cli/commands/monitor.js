...
export const monitorCommand = new Command().description('Start live monitoring dashboard').option('-i, --interval <seconds>', 'Update interval in seconds', '2').option('-c, --compact', 'Compact view mode').option('--no-graphs', 'Disable ASCII graphs').option('--focus <component:string>', 'Focus on specific component').action(async (options)=>{
...
let Dashboard = class Dashboard {
    ...
    renderFooter() {
        console.log('─'.repeat(80));
        console.log(chalk.gray('Press Ctrl+C to exit • Update interval: ') + chalk.yellow(`${this.options.interval}s`));
    }
    renderError(error) {
        console.clear();
    ...
    generateMockEvents() {
        ...
        return Array.from({
            ...
            return {
                ...event,
                timestamp: Date.now() - i * Math.random() * 300000,
    ...
    checkAlerts(data) {
        ...
        this.alerts = [
            ...this.alerts,
            ...newAlerts
        ].filter((alert)=>Date.now() - alert.timestamp < 300000).slice(-10);
    }
