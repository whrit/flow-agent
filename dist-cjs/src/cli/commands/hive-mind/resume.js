...
export const resumeCommand = new Command('resume').description('Resume paused hive mind sessions').option('-s, --session <id>', 'Resume specific session by ID').action(async (options)=>{
    ...
    try {
        if (options.session) {
            ...
            console.log(chalk.gray(`Swarm: ${session.swarm_name || 'N/A'}`));
