...
export const initCommand = new Command('init').description('Initialize a new Hive Mind swarm').option('-t, --topology <type>', 'Swarm topology (mesh, hierarchical, ring, star)', 'hierarchical').option('-m, --max-agents <number>', 'Maximum agents in swarm', '8').option('-n, --name <string>', 'Swarm name', 'hive-mind-' + Date.now()).option('-q, --queen-mode <mode>', 'Queen coordination mode (centralized, distributed)', 'centralized').option('--memory-ttl <seconds>', 'Default memory TTL in seconds', '86400').option('--consensus-threshold <percent>', 'Consensus threshold percentage', '0.66').option('--auto-spawn', 'Automatically spawn initial agents', false).action(async (options)=>{
