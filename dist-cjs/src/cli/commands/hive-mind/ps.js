...
export const psCommand = new Command('ps').description('Show active hive mind sessions and processes').option('-a, --all', 'Show all sessions including stopped ones').option('-v, --verbose', 'Show detailed process information').action(async (options)=>{
    ...
    try {
        ...
        for (const session of sessions){
            ...
            const statusColor = session.status === 'active' ? chalk.green : session.status === 'paused' ? chalk.yellow : session.status === 'stopped' ? chalk.red : chalk.gray;
            table.push([
