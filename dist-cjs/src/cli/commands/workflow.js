...
export const workflowCommand = new Command().name('workflow').description('Execute and manage workflows').action(()=>{
...
async function listWorkflows(options) {
    try {
        ...
        for (const workflow of workflows){
            ...
            const duration = workflow.completedAt ? formatDuration(workflow.completedAt.getTime() - workflow.startedAt.getTime()) : formatDuration(Date.now() - workflow.startedAt.getTime());
            table.push([
...
function displayWorkflowStatus(execution) {
    ...
    const duration = execution.completedAt ? formatDuration(execution.completedAt.getTime() - execution.startedAt.getTime()) : formatDuration(Date.now() - execution.startedAt.getTime());
    ...
    for (const taskExec of execution.tasks){
        ...
        const duration = taskExec.completedAt && taskExec.startedAt ? formatDuration(taskExec.completedAt.getTime() - taskExec.startedAt.getTime()) : taskExec.startedAt ? formatDuration(Date.now() - taskExec.startedAt.getTime()) : '-';
        table.push([
