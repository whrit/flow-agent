...
let TruthMonitoringServer = class TruthMonitoringServer extends EventEmitter {
    constructor(options = {}){
        super();
    ...
    broadcastTruthEvent(event) {
        ...
        const matchingSubscriptions = Array.from(this.subscriptions.values()).filter((sub)=>this.eventMatchesFilter(truthEvent, sub.filter));
        ...
        this.emit('truth_event_broadcast', {
            event: truthEvent,
    ...
    validateSubscriptionFilter(filter = {}) {
        return {
            event_types: Array.isArray(filter.event_types) ? filter.event_types : [
                'truth_change',
                'verification_update',
                'system_alert'
            ],
            severity_levels: Array.isArray(filter.severity_levels) ? filter.severity_levels : [
                'low',
                'medium',
                'high',
                'critical'
    ...
    calculateServerMetrics() {
        ...
        return {
            ...
            client_stats: {
                ...
                average_connection_age: clients.length > 0 ? clients.reduce((sum, c)=>sum + (now - c.connectedAt), 0) / clients.length : 0
            },
            subscription_stats: {
    ...
    getMostPopularEventTypes() {
        ...
        return Object.entries(eventTypeCounts).sort(([, a], [, b])=>b - a).slice(0, 5).map(([type, count])=>({
                type,
                count
            }));
    }
...
//# sourceMappingURL=truth-monitor.js.map