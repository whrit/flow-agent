...
function calculateMetrics() {
    ...
    return {
        ...
        verification_rate: total > 0 ? total / (uptime / 1000 / 3600) : 0,
...
function getActiveVerifications() {
    return Array.from(verificationStore.verifications.values()).filter((v)=>v.status === 'pending').slice(0, 10);
}
function getRecentEvents(limit = 10) {
    return verificationStore.events.sort((a, b)=>b.timestamp - a.timestamp).slice(0, limit);
}
...
function calculateAverageConfidence() {
    const verifications = Array.from(verificationStore.verifications.values()).filter((v)=>v.status !== 'pending');
...
function calculateTrends(events) {
    ...
    return {
        ...
        peak_hour: Object.keys(hourlyBuckets).reduce((a, b)=>hourlyBuckets[a] > hourlyBuckets[b] ? a : b, '0')
    };
}
