...
export class CheckpointManager {
    ...
    storagePath;
    constructor(storagePath = '.claude-flow/checkpoints'){
    ...
    async createPreExecutionCheckpoint(agentId, taskId, description) {
        const validations = [
            {
        ...
        const checkpointId = await this.createCheckpoint(`Pre-execution: ${description}`, 'task', agentId, taskId, validations);
        await this.executeValidations(checkpointId, validations);
        return checkpointId;
    }
    async createPostExecutionCheckpoint(agentId, taskId, description, truthScore) {
        const validations = [
            {
        ...
        const checkpointId = await this.createCheckpoint(`Post-execution: ${description}`, 'task', agentId, taskId, validations);
        await this.executeValidations(checkpointId, validations);
        return checkpointId;
    }
    ...
    async captureSystemState(scope, agentId, taskId) {
        ...
        const systemState = stateScope.include_agents ? await this.captureSystemState_Component() : {};
        const taskStates = stateScope.include_tasks ? await this.captureTaskStates(stateScope.task_filter) : new Map();
        const memoryState = stateScope.include_memory ? await this.captureMemoryState() : {};
        const fileSystemState = stateScope.include_filesystem ? await this.captureFileSystemState() : {};
        const databaseState = stateScope.include_database ? await this.captureDatabaseState() : {};
    ...
    async captureAgentStates(agentFilter) {
    ...
    async captureSystemState_Component() {
        return {
    ...
    async captureTaskStates(taskFilter) {
    ...
    calculateStateChecksum(stateData) {
        const stateString = JSON.stringify(stateData, null, 0);
        return createHash('sha256').update(stateString).digest('hex');
    }
    async executeValidations(checkpointId, validations) {
        const checkpoint = this.checkpointStore.get(checkpointId);
    ...
    async executeValidationCommand(command, expectedResult) {
        switch(command){
