...
export async function initializeTaskManagement(config = {}) {
...
export async function createTaskTodos(objective, options = {}, coordinator) {
    if (!coordinator) {
...
export async function launchParallelAgents(tasks, coordinator) {
    if (!coordinator) {
...
export async function storeCoordinationData(key, value, options = {}, coordinator) {
    if (!coordinator) {
        throw new Error('TaskCoordinator instance required for memory storage');
    }
    await coordinator.storeInMemory(key, value, options);
}
export async function retrieveCoordinationData(key, namespace, coordinator) {
    if (!coordinator) {
