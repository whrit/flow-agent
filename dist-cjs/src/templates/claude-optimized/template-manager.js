...
const commands = {
    install: ()=>execSync('node install-template.js', {
            stdio: 'inherit'
        }),
    validate: ()=>execSync('node validate-template.js', {
            stdio: 'inherit'
        }),
    deploy: (targetPath)=>{
        if (!targetPath) {
        ...
        execSync(`node deploy-to-project.js "${targetPath}"`, {
            stdio: 'inherit'
        });
    },
    info: ()=>{
    ...
    update: ()=>{
        ...
        execSync('node install-template.js', {
            stdio: 'inherit'
        });
        console.log('2. Validating installation...');
        execSync('node validate-template.js', {
            stdio: 'inherit'
        });
