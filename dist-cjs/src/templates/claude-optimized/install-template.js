...
for (const [category, info] of Object.entries(manifest.categories)){
    const copied = manifest.files.filter((f)=>f.category === category && fs.existsSync(path.join(DEST_DIR, f.destination))).length;
    console.log(`  ${category}: ${copied}/${info.count} files`);
}
if (errorCount === 0) {
    console.log('\n✅ Template installation completed successfully!');
    const timestamp = new Date().toISOString();
    fs.writeFileSync(path.join(__dirname, '.installed'), `Installed: ${timestamp}\nVersion: ${manifest.version}\n`);
} else {
