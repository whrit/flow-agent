...
for (const cmd of sparcCommands.slice(0, 3)){
    ...
    if (fs.existsSync(filePath)) {
        const content = fs.readFileSync(filePath, 'utf8');
        test(`${cmd.destination} has proper structure`, content.includes('## Instructions') || content.includes('You are'));
    }
}
...
for (const testFile of testFiles.slice(0, 3)){
    ...
    if (fs.existsSync(filePath)) {
        const content = fs.readFileSync(filePath, 'utf8');
        test(`${testFile.destination} has test structure`, content.includes('describe') || content.includes('test') || content.includes('it'));
    }
}
