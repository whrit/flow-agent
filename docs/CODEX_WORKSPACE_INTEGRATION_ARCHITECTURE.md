...
async function verifyMcpConfiguration() {
  ...
  try {
    ...
    if (missingServers.length > 0) {
      ...
      console.log(chalk.green('  codex mcp add claude-flow npx bot-flow@alpha mcp start'));
...
async function ensureMcpServers() {
  ...
  try {
    ...
    if (!mcpList.includes('claude-flow')) {
      ...
      execSync('codex mcp add claude-flow npx bot-flow@alpha mcp start', { stdio: 'inherit' });
    }
...
codex mcp add claude-flow npx bot-flow@alpha mcp start
  ...
  checks.push({
    ...
    check: async () => {
      ...
      return {
        ...
        message: 'Run: codex mcp add claude-flow npx bot-flow@alpha mcp start',
        critical: false
      };
    }
  });
