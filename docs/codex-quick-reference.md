...
```bash
npx flow-agent hive-mind start "Your objective here" \
  --codex \
...
npx flow-agent hive-mind start "Build feature X" \
  --codex \
...
npx flow-agent hive-mind start "Deploy to staging" \
  --codex \
...
npx flow-agent hive-mind start "Refactor critical code" \
  --codex \
...
```bash
npx flow-agent hive-mind status
npx flow-agent hive-mind list
```

### Resume Session
```bash
npx flow-agent hive-mind resume <session-id>
```
...
```bash
npx flow-agent hive-mind start \
...
# 3. Test run (dry-run)
npx flow-agent hive-mind start "test" --codex --dry-run
