...
npx bot-flow@alpha task create general "Analyze codebase" --provider codex

# Swarm coordination
npx bot-flow@alpha swarm init --provider codex --topology mesh

# Agent spawning
npx bot-flow@alpha agent spawn researcher --provider codex

# SPARC workflow
npx bot-flow@alpha sparc run architect "Design API" --provider codex
```
...
npx bot-flow@alpha hive-mind spawn "Build REST API" \
...
npx bot-flow@alpha task create general "Task description" --provider codex
